...
function joinWithdrawRoom(socket, withdrawId) {
  if (!withdrawId) return;
  socket.join(`withdraw:${withdrawId}`);
}

io.on("connection", (socket) => {
  try {
    const q = socket.handshake.query || {};
    const userId = String(q.userId || q.uid || q.clientId || "").trim();
    const role = String(q.role || "").trim().toLowerCase();
    const withdrawId = String(q.withdrawId || "").trim();

    if (userId) {
      socket.join(`user:${userId}`);
      socket.join(`withdraw-user:${userId}`);
    }

    if (role === "admin" || q.admin === "true") {
      socket.join("admins");
    }

    if (withdrawId) {
      joinWithdrawRoom(socket, withdrawId);
    }

    socket.on("withdraw:join", (payload = {}) => {
      const wid = String(payload.withdrawId || payload.id || "").trim();
      const uid = String(payload.userId || payload.uid || "").trim();
      if (uid) socket.join(`user:${uid}`);
      if (wid) joinWithdrawRoom(socket, wid);
    });

    socket.on("withdraw:leave", (payload = {}) => {
      const wid = String(payload.withdrawId || payload.id || "").trim();
      if (wid) socket.leave(`withdraw:${wid}`);
    });
  } catch (err) {
...
async function loadWithdrawsForUser(userId, status = null) {
  const query = { userId: String(userId) };
  if (status) query.status = status;
  return await Withdraw.find(query).sort({ createdAt: -1 }).lean().exec().catch(() => []);
}

async function loadAllWithdraws(limit = 1000, status = null) {
  const query = {};
  if (status) query.status = status;
...
function pushWithdrawMessage(doc, sender, message) {
...
function emitWithdrawEvents(withdrawDoc) {
  if (!withdrawDoc) return;
  const plain = withdrawDoc.toObject ? withdrawDoc.toObject() : withdrawDoc;
  const userId = String(plain.userId || "").trim();
  const id = String(plain._id || "").trim();

  io.emit("admin:new-withdraw", plain);
  if (userId) {
    io.emit(`withdraw-request:${userId}`, plain);
    io.emit(`withdraw-update:${userId}`, plain);
    io.to(`user:${userId}`).emit("withdraw:update", plain);
  }
  if (id) {
    io.to(`withdraw:${id}`).emit("withdraw:update", plain);
    io.emit(`withdraw:${id}`, plain);
  }
  io.to("admins").emit("admin:withdraw-update", plain);
}
...
/* OBTENER UN RETIRO EN ESPECÍFICO */
app.get("/api/withdraw/:id", async (req, res) => {
  try {
    const withdraw = await Withdraw.findById(req.params.id).lean().catch(() => null);
    if (!withdraw) {
      return res.status(404).json({ ok: false, msg: "Retiro no encontrado" });
    }
    return res.json({ ok: true, withdraw });
  } catch (err) {
    console.error("withdraw detail error:", err);
    return res.status(500).json({ ok: false, msg: "Error obteniendo retiro" });
  }
});

/* MENSAJE CLIENTE A RETIRO */
app.post("/api/withdraw/message", async (req, res) => {
  try {
    ...
    if (userId && String(withdraw.userId) !== String(userId)) {
      return res.status(403).json({ ok: false, msg: "No corresponde al usuario" });
    }

    pushWithdrawMessage(withdraw, "client", message);
    withdraw.updatedAt = new Date();
    await withdraw.save();

    emitWithdrawEvents(withdraw);

    return res.json({ ok: true, msg: "Mensaje enviado", withdraw });
  } catch (err) {
    console.error("withdraw client message error:", err);
    return res.status(500).json({ ok: false, msg: "Error enviando mensaje" });
  }
});
...
app.get("/api/admin/withdraws", ensureAdminAuth, async (req, res) => {
  try {
    ...
    const data = await loadAllWithdraws(limit, status);
...
/* CONSULTA INDIVIDUAL POR USUARIO (SOLO INDIVIDUAL) */
app.get("/api/admin/withdraws/:userId", ensureAdminAuth, async (req, res) => {
  try {
    const data = await loadWithdrawsForUser(req.params.userId, null);
    return res.json({
      ok: true,
      data,
    });
  } catch (err) {
    console.error("GET withdraws by user error:", err);
...
app.get("/api/admin/withdraw/:id", ensureAdminAuth, async (req, res) => {
  try {
    ...
    console.error("admin withdraw detail error:", err);
    return res.status(500).json({ ok: false, msg: "Error obteniendo retiro" });
  }
});

/* AGREGAR MENSAJE ADMIN AL RETIRO */
app.post("/api/admin/withdraw/message", ensureAdminAuth, async (req, res) => {
  try {
    ...
    pushWithdrawMessage(withdraw, "admin", message);
    withdraw.adminNote = message;
    withdraw.updatedAt = new Date();
    await withdraw.save();

    emitWithdrawEvents(withdraw);

    return res.json({ ok: true, msg: "Mensaje guardado", withdraw });
  } catch (err) {
    console.error("admin withdraw message error:", err);
    return res.status(500).json({ ok: false, msg: "Error guardando mensaje" });
  }
});
...
app.post("/api/admin/withdraw/counter", ensureAdminAuth, async (req, res) => {
  try {
    ...
    withdraw.offerAmount = Number(offerAmount || 0);
    withdraw.adminNote = adminNote || "";

    pushWithdrawMessage(
      withdraw,
      "admin",
      adminNote || `Nueva propuesta enviada: ${offerAmount}`
    );
    ...
    return res.status(500).json({
      ok: false,
      msg: "Error creando contraoferta",
    });
  }
});
...
app.post("/api/withdraw/accept-offer", async (req, res) => {
  try {
    ...
    withdraw.amount = withdraw.offerAmount;
    withdraw.status = "pending";

    pushWithdrawMessage(withdraw, "client", "Oferta aceptada");
...
app.post("/api/admin/withdraw/approve", ensureAdminAuth, async (req, res) => {
  try {
    ...
    const local = await localWithdraw({
      userId,
      amount,
      note: adminNote || `Retiro aprobado #${id}`,
    });
    ...
    pushWithdrawMessage(
      w,
      "admin",
      adminNote || "Retiro aprobado"
    );

    await w.save();

    emitWithdrawEvents(w);

    io.emit(
      `withdraw-update:${userId}`,
      w
    );
